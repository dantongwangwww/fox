<template>
  <div ref="floatDrag" id="floatDrag" class="float-position"
    :style="{ top: data.top + 'px', right: data.right + 'px !important' }" @touchmove.prevent @mousemove.prevent
    @mousedown="mouseDown" @mouseup="mouseUp">
    <div class="content">
      <div class="icon-box">
        <SvgIcon class="btn-icon" name="tree" color="#666" height="16px" />
      </div>
    </div>
  </div>
</template>
    ...
    const mouseDown = (e) => {
      ...
      document.onmousemove = function(e) {
        ...
        if (
          ...
          floatDragDom.value.style.transition = 'all 0.1s';
          return;
        }
